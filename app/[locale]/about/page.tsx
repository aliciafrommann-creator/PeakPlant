    ...
    <div style={{ fontFamily: PP, background: '#ffffff', color: '#1A1A1A', minHeight: '100vh' }}>
      ...
      <section style={{ padding: '10rem 2.5rem 5rem', maxWidth: 800, margin: '0 auto' }}>
        ...
        <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, ease: [0.16, 1, 0.3, 1], delay: 0.2 }}
          ...
          alicia frommann. {isDE ? 'gründerin' : 'founder'}, peakplant.
        </motion.p>
      </section>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.8rem', paddingTop: '5rem' }}>
          ...
          <motion.div initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            ...
            <p style={{ fontSize: '0.65rem', letterSpacing: '0.18em', textTransform: 'uppercase', opacity: 0.4, marginBottom: '1.25rem', fontFamily: PP }}>
              {isDE ? 'was mein denken verändert hat' : 'what changed my thinking'}
            </p>
            {books.map(b => (
      ...
      <section style={{ borderTop: '1px solid #e8e8e8', padding: '7rem 2.5rem', textAlign: 'center' }}>
        ...
        <Link href={localHref('/philosophy')} style={{ fontFamily: PP, fontSize: '0.7rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: '#1A1A1A', opacity: 0.5, textDecoration: 'none', borderBottom: '1px solid rgba(26,26,26,0.3)', paddingBottom: 3 }}>
          {isDE ? 'unsere philosophie lesen →' : 'Read our philosophy →'}
        </Link>
      </section>
    </div>
  )
