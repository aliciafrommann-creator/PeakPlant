...
# 1. Geänderte VERFOLGTE Dateien beiseitelegen, statt am Pull zu scheitern.
#    `git stash` wirft nichts weg: `git stash list` zeigt es, `git stash pop`
#    holt es zurück.
#
#    OHNE `-u`, und das ist wichtig: Der erste Testlauf dieses Skripts hat
#    sich SELBST weggeräumt, weil es noch nicht eingecheckt war. Unverfolgte
#    Dateien sind fast immer die Arbeit eines Menschen — Notizen, Screenshots,
#    ein halbfertiger Entwurf. Ein Startskript darf so etwas nicht anfassen,
#    auch nicht wiederherstellbar.
if [ -n "$(git -C "$WURZEL" status --porcelain --untracked-files=no)" ]; then
  echo "  · geänderte Dateien beiseitegelegt (git stash list zeigt sie)"
  git -C "$WURZEL" stash push -m "automatisch vor dem Start $(date +%H:%M)" >/dev/null 2>&1
fi
