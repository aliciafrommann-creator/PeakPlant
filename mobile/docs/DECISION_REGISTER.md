...
| PP-022 | Revenue | "Moments to do together" can link to **verified local partner places** only after a real venue agreement exists. Until then, places are live provider results or generic search intents. Partner perks may be added later, transparently, never required to participate. Aggregate, privacy-safe — no per-user commercial profiling. | DECIDED |
...
| PP-026 | Editions | **Friends are a three-way split, not "all out".** (1) Friends *spaces* stay — same diary/model, no extra cost, used with couples-neutral editions. (2) A dedicated friends *product line* (own physical editions) is **not** shipped at launch — it would dilute the couples/intimacy brand and double SKU + marketing. (3) A friends *sub-brand* is backlog, revisited once the couples line proves out. Launch product = the **9 couples editions** only (catalog `SEED_EDITIONS`); only `edition-01` ships with cards, the rest are roadmap `upcoming`. | DECIDED |
| PP-027 | Date Discovery | **Date Discovery is a sanctioned extension of the existing "moments to do together" pillar** — it unifies `missions` (curated activity ideas) + `localShops` (live/provider places and generic place intents) into a single **Discover** surface. It is not a pivot; it reuses `lib/together.ts`, `lib/ai/`, and the verified-partner guardrail in PP-022. The Discover tab replaces the Us tab as the primary landing surface, with the diary remaining the gravitational core. | DECIDED |
| PP-028 | Navigation | Navigation restructured to **Discover · Moments · Scan · Grow** (four tabs). The former Us tab is hidden (route preserved) and all routing redirects to Discover. Challenges and streaks are demoted to Discover's secondary links, not top-level tabs. | DECIDED |
| PP-029 | Discovery honesty | The recommender **never fabricates venues, facts, or availability**. Every fact on a recommendation card carries a provenance label (`curated`, `estimated`, `ai-interpretation`, `needs-confirmation`, `verified-live`). Static place entries are search prompts, not venue claims. Verified partner places are **never ranked higher** for being partners — ranking is not for sale (extends PP-016). | DECIDED |
| PP-030 | Discovery privacy | DateConstraints are **ephemeral per-request** — never stored as a durable profile. Personalization uses only explicit signals (goals set at onboarding, per-session shortcut chips). A transparent **Personalization screen** lets users see, understand, and clear every signal. Session chips reset on navigation; goals can be cleared at any time. | DECIDED |
