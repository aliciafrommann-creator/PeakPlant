...
| PP-027 | Date Discovery | **Date Discovery is a sanctioned extension of the existing "moments to do together" pillar** — it unifies `missions` (curated activity ideas) + `localShops` (partner venues) into a single **Discover** surface. It is not a pivot; it reuses `lib/together.ts`, `lib/ai/`, and the existing partner-place model (PP-022). The Discover tab replaces the Us tab as the primary landing surface, with the diary remaining the gravitational core. | DECIDED |
...
| PP-029 | Discovery honesty | The recommender **never fabricates venues, facts, or availability**. Every fact on a recommendation card carries a provenance label (`curated`, `estimated`, `ai-interpretation`, `needs-confirmation`; never `verified-live` in the MVP). The recommender returns `[]` when over-constrained rather than inventing results. Partner places are **never ranked higher** for being partners — ranking is not for sale (extends PP-016). | DECIDED |
